...
try {
    ...
    if ($action === 'fix_misassigned') {
        // ─────────────────────────────────────────────────────────
        // ปิดใช้งานถาวร 2026-08-26
        // "คนขาย ≠ เจ้าของ" ไม่ใช่ข้อมูลผิดที่ต้องซ่อม แต่คือกรณีที่คนอื่น
        // ไปขายลูกค้าของเจ้าของเดิม ซึ่งเจ้าของเดิมต้องได้ถือครองต่อ
        // จนหมดสิทธิ์ตามกติกาตะกร้า (ดู BasketRoutingServiceV2 กฎ P4/P5/P6)
        // การสแกน (action=misassigned) ยังใช้ได้ตามปกติ ไว้ดูว่าใครไปขายลูกค้าใคร
        // ถ้าต้องการโอนเจ้าของจริง ให้ใช้เมนูโอน/แจกลูกค้า ที่บันทึกประวัติได้
        // ─────────────────────────────────────────────────────────
        http_response_code(403);
        echo json_encode([
            'success' => false,
            'disabled' => true,
            'error' => 'ปิดใช้งานถาวร: ห้ามย้ายเจ้าของลูกค้าที่มีผู้ดูแลอยู่แล้ว ถ้าต้องการโอนจริงให้ใช้เมนูโอน/แจกลูกค้าที่บันทึกประวัติได้'
        ], JSON_UNESCAPED_UNICODE);
        exit;
    }

    if (false && $action === 'fix_misassigned') {
        $body = json_decode(file_get_contents('php://input'), true);
