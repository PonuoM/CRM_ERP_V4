...
try {
    ...
    try {
        ...
        if (!$page) {
            throw new Exception('Page not found or you do not have permission to delete it');
        }

        // Prevent deletion of certain page types if needed (optional)
        if ($page['page_type'] === 'pancake') {
            throw new Exception('Cannot delete Pancake-synced pages. Please remove them from Pancake instead.');
        }

        // Check if page has users assigned
        $userCountStmt = $pdo->prepare("
            SELECT COUNT(*) as user_count FROM marketing_user_page
            WHERE page_id = ?
        ");
        $userCountStmt->execute([$pageId]);
        $userCount = $userCountStmt->fetch()['user_count'];

        if ($userCount > 0) {
            // Option 1: Remove user assignments first
            $removeUsersStmt = $pdo->prepare("
                DELETE FROM marketing_user_page WHERE page_id = ?
            ");
            $removeUsersStmt->execute([$pageId]);

            // Option 2: Prevent deletion and show error
            // Uncomment the following lines if you want to prevent deletion when users are assigned
            /*
            throw new Exception("Cannot delete page '{$page['name']}' because it has {$userCount} users assigned. Please reassign or remove users first.");
            */
        }
        ...
        json_response([
            ...
            'data' => [
                ...
                'removed_users' => $userCount > 0 ? $userCount : 0
            ],
            'message' => "Page '{$page['name']}' deleted successfully" . ($userCount > 0 ? " (removed {$userCount} user assignments)" : "")
        ]);
