...
try {
    ...
    foreach ($items as $item) {
        ...
        // Upsert stock (key: warehouse + product + lot)
        $lotKey = $lotNumber ?? '';
        $stmt = $pdo->prepare("SELECT id, quantity FROM inv2_stock WHERE warehouse_id = ? AND product_id = ? AND COALESCE(lot_number,'') = ?");
        ...
        // Log movement
        $pdo->prepare("INSERT INTO inv2_movements (warehouse_id, product_id, lot_number, movement_type, quantity, reference_type, reference_doc_number, notes, images, created_by, company_id) VALUES (?,?,?,?,?,?,?,?,?,?,?)")
            ->execute([$warehouseId, $productId, $lotNumber, $movementType, $qty, 'adjustment', $docNumber, $itemNotes, json_encode($images), $userId, $companyId]);
    }
