-- Migration: Allow COD collected_amount to exceed collection_amount
-- Instead of hard-blocking, the frontend will show a warning and let the user confirm
-- Date: 2026-02-21
...
BEGIN
    ...
    -- REMOVED: Hard block on excess collection (2026-02-21)
    -- Previous: IF NEW.collected_amount + NEW.waived_amount > NEW.collection_amount THEN SIGNAL ...
    -- Now handled by frontend warning flow — excess is allowed but warned
