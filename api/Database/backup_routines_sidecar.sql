-- Sidecar stored procedures for restore drills.
-- These four exist in the repo. generate_test_customers and UpdateCustomerStatsBatch
-- are NOT in git — paste phpMyAdmin Export (Routines only) into
-- scripts/backup/routines_extra.sql.example (copy to routines_extra.sql locally).
-- Run this file on a TEST database only. Never against primacom_mini_erp on the host.
