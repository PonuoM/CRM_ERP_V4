        ...
        case 'POST':
            ...
            $stmt = $pdo->prepare("
                ...
                (basket_key, basket_name, min_order_count, max_order_count, 
                 ...
                 extend_days_per_appointment, max_total_days, extend_days_sales_amount_threshold, extend_days_sales_reward, max_extend_appointments, advance_transfer_days)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
...
function handleReclaimCustomers($pdo, $companyId)
{
    ...
    try {
        ...
        $sessionStmt = $pdo->prepare("INSERT INTO distribution_sessions (company_id, distributed_by, distribution_mode, min_call_minutes, total_customers, created_at, agent_snapshot, source_basket, tag_id) VALUES (?, ?, 'Bulk Reclaim', NULL, 0, NOW(), NULL, 'Multiple Baskets', ?)");
