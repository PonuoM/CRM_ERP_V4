...
function handlePost($pdo) {
    ...
    if (empty($input['order_id'])) json_response(['ok' => false, 'error' => 'order_id is required'], 400);
    if (empty($input['user_id'])) json_response(['ok' => false, 'error' => 'user_id is required'], 400);
    if (!isset($input['result_status'])) json_response(['ok' => false, 'error' => 'result_status required'], 400);
...
function handlePut($pdo, $id) {
    ...
    if (isset($input['amount_collected'])) { $updates[] = "amount_collected = ?"; $params[] = (float)$input['amount_collected']; }
    if (isset($input['result_status'])) { $updates[] = "result_status = ?"; $params[] = (int)$input['result_status']; }
    if (isset($input['is_complete'])) { $updates[] = "is_complete = ?"; $params[] = (int)$input['is_complete']; }
    if (isset($input['note'])) { $updates[] = "note = ?"; $params[] = $input['note']; }
    if (isset($input['slip_id'])) { $updates[] = "slip_id = ?"; $params[] = $input['slip_id']; }
    
    if (empty($updates)) json_response(['ok' => false, 'error' => 'No updates'], 400);
