...
try {
    ...
    $pdo->exec("DELETE FROM basket_config WHERE id IN (9997, 9998, 9999)");
    
    $pdo->exec("
        INSERT INTO basket_config 
        (id, company_id, basket_key, basket_name, fail_after_days, on_fail_basket_key, extend_days_sales_amount_threshold, extend_days_sales_reward, extend_days_per_appointment, max_total_days, is_active)
        VALUES 
        (9998, 9999, 'fallback_basket', 'Fallback Basket', 0, NULL, 0, 0, 0, 0, 1),
        (9999, 9999, 'test_basket_cron', 'Test Basket Sales', 30, 'fallback_basket', 50000, 30, 0, 90, 1),
        (9997, 9999, 'test_basket_appt', 'Test Basket Appt', 30, 'fallback_basket', 0, 0, 30, 90, 1)
    ");
    echo "✅ Mock Basket Configurations inserted.\n";

    // 3. Clear Old Test Data
    $pdo->exec("DELETE FROM customers WHERE company_id = 9999");
    $pdo->exec("DELETE FROM basket_transition_log WHERE from_basket_key IN ('9999', '9997')");
    $pdo->exec("DELETE FROM appointments WHERE id IN (9999904, 9999905)");
