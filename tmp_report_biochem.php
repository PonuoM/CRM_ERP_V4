...
echo "=== Bio Products ===\n";
$res = $conn->query("SELECT id, name_th, category FROM products WHERE name_th LIKE '%ชีว%' OR category LIKE '%ชีว%'");
$bio_product_ids = [];
if ($res) {
...
if ($res) {
    ...
    while($r = $res->fetch_assoc()){
        printf("%-25s | %-12s | %-12.2f | %-12.2f\n", 
            mb_substr($r['user_name'], 0, 25), 
            $r['total_orders'], 
            $r['total_sales'], 
            $r['bio_sales']
        );
