...
const App: React.FC = () => {
    ...
    const handleProcessOrders = async (ordersToProcess: { id: string; customerId?: string; creatorId?: number }[]) => {
        ...
        if (true) {
            ...
            // Process customer updates (Only if Creator is Telesale/Supervisor)
            ...
            Object.entries(customerUpdates).forEach(async ([customerId, data]) => {
                try {
                    // Check creator role
                    const creatorUser = companyUsers.find(u => u.id === data.creatorId);
                    if (!creatorUser) {
                        console.warn(`Creator ${data.creatorId} not found, skipping customer update`);
                        return;
                    }
                    ...
                    if (!isTelesale) {
                        console.log(`Creator ${data.creatorId} is (${creatorUser.role}), skipping customer update`);
                        return;
                    }
