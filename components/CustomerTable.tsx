  ...
  const TagColumn: React.FC<{customer: Customer}> = ({ customer }) => {
    ...
    return (
        <div className="flex items-center flex-wrap gap-1">
            {visibleTags.map(tag => (
                <span key={tag.id} className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${tag.type === TagType.System ? 'bg-purple-100 text-purple-800' : 'bg-green-100 text-green-800'}`}>
                    {tag.name}
                </span>
            ))}
            {hiddenCount > 0 && (
                <div className="group relative">
                    ...
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 w-48 p-2 bg-gray-800 text-white text-xs rounded-md opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                        {customer.tags.slice(2).map(t => t.name).join(', ')}
                    </div>
                </div>
            )}
    ...
    <div className="bg-white rounded-lg shadow">
      <div className="overflow-x-auto">
        ...
        <thead className="text-xs text-gray-700 uppercase bg-gray-50">
          <tr>
            <th scope="col" className="px-6 py-3">วันที่ได้รับ</th>
        ...
        <tbody>
          {currentCustomers.length > 0 ? currentCustomers.map((customer) => {
            const remainingTime = getRemainingTimeRounded(customer.ownershipExpires);
            return (
              <tr key={customer.id} className="bg-white border-b hover:bg-gray-50">
                <td className="px-6 py-4">{new Date(customer.dateAssigned).toLocaleDateString('th-TH')}</td>
                ...
                <td className={`px-6 py-4 ${remainingTime.color}`}>{remainingTime.text}</td>
                <td className="px-6 py-4">
                  <span className={`px-2.5 py-1 text-xs font-medium rounded-full ${statusColorMap[customer.lifecycleStatus] || 'bg-gray-100 text-gray-800'}`}>
