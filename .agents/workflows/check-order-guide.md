...
หน้า **ตรวจสอบคำสั่งซื้อ** (`pages/CheckOrderPage.tsx`) เป็นศูนย์กลางสำหรับตรวจสอบความถูกต้องของออเดอร์ แบ่งเป็น 6 tab:
...
| ตรวจสอบไฟล์ COD | emerald | อัปโหลด CSV ตรวจว่า tracking ใดจะทำให้ collected > collection_amount |
...
| ไฟล์ | บทบาท |
|------|--------|
| `pages/CheckOrderPage.tsx` | Main page — 6 tab components |
...
| `api/Orders/check_cod_file.php` | ตรวจสอบไฟล์ COD ว่า tracking ไหนจะทำให้ collected เกิน collection_amount |
...
### 5. CODFileCheckTab (ตรวจสอบไฟล์ COD)

**จุดประสงค์:**
ตรวจสอบไฟล์รายละเอียดโอนเงิน COD (CSV) ก่อนนำเข้า ว่ามี tracking ไหนที่จะทำให้ trigger `trg_order_boxes_bi_enforce` ระเบิด (เพราะ `collected_amount + waived_amount > collection_amount`)

**สาเหตุที่พบบ่อย:**
ออเดอร์ถูก mark เป็นตีกลับ (Returned) → `collection_amount` ใน `order_boxes` ถูกปรับเป็น 0 → แต่ขนส่งยังโอนเงิน COD มาให้

**Flow:**
1. เลือกไฟล์ CSV (รูปแบบ: `tracking_number,cod_amount` ไม่มี header)
2. ระบบ parse ไฟล์ client-side แล้วแสดงจำนวนรายการที่อ่านได้
3. กดปุ่ม **"ตรวจสอบ"** → ส่ง items ไป `check_cod_file.php`
4. Backend query `order_tracking_numbers` → `order_boxes` → `orders` เพื่อจำลองว่า ถ้า import เข้าแล้วยอด collected จะเกินไหม
5. แสดง Summary Cards (5 การ์ด): Total, OK, No Box, Non-COD, Exceed
6. ถ้ามี tracking ที่จะเกิน → แสดงตารางพร้อม Order ID, สถานะ, ยอดต่างๆ

**API:** `apiFetch('Orders/check_cod_file.php', { method: 'POST', body: { items, company_id } })`

**Response:**
```json
{
  "problems": [{ "tracking_number", "order_id", "collection_amount", "current_collected", "waived_amount", "csv_cod_amount", "exceed_by", "order_status", ... }],
  "summary": { "total", "ok", "no_box", "non_cod", "exceed" }
}
```
...
| `apiFetch('Orders/check_cod_file.php', opts)` | `Orders/check_cod_file.php` | POST |
