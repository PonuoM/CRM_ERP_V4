...
4. `getExportOrderIds(exportId)` → ดึง order IDs → fetch ข้อมูลออเดอร์ → นำไปเข้า `mapApiOrderToModel()` เพื่อแปลงให้โครงสร้างข้อมูลตรงกับ Export ปกติเป๊ะ (อัปเดตล่าสุด: ทิ้ง Inline Mapping แบบเก่าที่ซ้ำซ้อน) → สร้างไฟล์ใหม่
