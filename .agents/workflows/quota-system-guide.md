...
| `api/Quota/quota.php` | Backend API ทั้งหมด (14 endpoints + global/scope support + summary_by_rate + bulk_confirm) |
| `types.ts` | TypeScript interfaces (4 ตัว: QuotaProduct, QuotaRateSchedule, QuotaAllocation, QuotaSummary) |
| `services/quotaApi.ts` | Service layer — frontend → API (15 functions รวม getSummaryByRate, bulkConfirmQuota) |
...
| `components/SingleDatePicker.tsx` | ปฏิทินเลือกวันเดียว (ใช้ใน modal สร้าง rate) |
| `components/DateRangePicker.tsx` | ปฏิทินเลือกช่วงวัน (ใช้สำหรับช่วงออเดอร์คำนวณ) |
...
| `summary` | `companyId, quotaProductId` | สรุปโควตาพนักงานทุกคนใน company (legacy, ใช้ product-based) |
| `summary_by_rate` | `companyId, rateScheduleId` (number หรือ `'all'`) | **สรุปโควตาตาม rate** — เลือก rate เดียว หรือ `'all'` = aggregate ทุก rate |
...
| `bulk_confirm_quota` | `rateScheduleId, userIds[], confirmedBy?, companyId` | **ยืนยันโควตาหลายคนพร้อมกัน** → loop confirm ทีละ user → return `{ confirmed, results[] }` |
...
| ฟังก์ชัน | ใช้ตอน | วิธีหา rate |
|----------|--------|------------|
| `calculateQuota(quotaProductId, userId)` | `calculate`, `summary` endpoints | ค้นหา latest rate จาก DB |
| `calculateQuotaByRate(rate, userId, companyId)` | `summary_by_rate` endpoint | **ใช้ rate ที่ส่งมาตรงๆ** (ไม่ค้นหา) |

### Helper functions:

| Helper | คำอธิบาย |
|--------|----------|
| `_calcSalesInPeriod(conn, userId, quotaProductId, dateCol, start, end)` | คำนวณยอดขายผ่าน quota_products.company_id |
| `_calcSalesInPeriodByCompany(conn, userId, companyId, dateCol, start, end)` | คำนวณยอดขายผ่าน companyId ตรงๆ (สำหรับ global rate ที่ไม่มี quota_product_id) |

### ขั้นตอน:

1. **ดึง latest rate** → `quota_rate_schedules` WHERE `effective_date ≤ NOW()` ORDER DESC LIMIT 1
   - `calculateQuota`: ค้นหา product-specific ก่อน → fallback global/scoped
   - `calculateQuotaByRate`: ใช้ rate ที่ส่งมาเลย (ไม่ค้นหา)
...
#### Tab 2: อัตราโควตา
- **แถบตัวกรอง (Filter Bar):** แทน dropdown เลือกสินค้าเดิม
  - **สินค้า:** Multi-select dropdown (เลือกได้หลายรายการ + 🌐 Global)
  - **ประเภท Rate:** dropdown เลือก reset/cumulative/confirm/ทั้งหมด
  - **วันที่มีผล:** ช่วงวันที่ (จาก/ถึง)
  - ปุ่ม "ล้างตัวกรอง" + แสดงจำนวน "แสดง X จาก Y รายการ"
- **Data loading:** โหลด rates จากทุก product + global ทีเดียว (`loadAllRates`) แล้ว filter ฝั่ง frontend
- **Rate card:** แสดง badge ชื่อสินค้า (📦) หรือ scope (🌐/📌) ทุก card
- **Badge สถานะ:**
  ...
  - `confirm`: แสดงได้ **หลายตัวพร้อมกัน** ตามช่วงใช้งาน:
    - 🟣 "ใช้งานอยู่" → `usage_start_date ≤ today ≤ usage_end_date`
...
- **Modal สร้าง/แก้ rate** (`max-w-3xl`, กว้างกว่าเดิม):
  - **ลำดับ input:**
    1. **โหมดโควตา** — radio buttons (reset/สะสม/กำหนดเอง) อยู่บนสุด
    2. **ยอดขาย + คำนวณจากออเดอร์ตาม** — แถวเดียวกัน (grid 2 cols)
    3. **สินค้าที่ใช้โควตานี้ได้** — multi-select dropdown (เลือกทั้งหมด/ล้าง + แสดง tags)
    4. **มีผลตั้งแต่วันที่** — `SingleDatePicker` (แสดงเฉพาะ reset/cumulative)
  - **Reset mode:** เลือก monthly (dropdown 1-28) หรือ interval (จำนวนวัน + วันเริ่มนับแถวเดียวกัน, ใช้ `SingleDatePicker`)
  - **Confirm mode:** แสดง info box + ช่วงออเดอร์คำนวณ (`DateRangePicker`) + วันเริ่ม/หมดอายุ (`SingleDatePicker` x2 แถวเดียวกัน) + checkbox รอ Admin ยืนยัน
  - **⚠️ Confirmation dialog:** เมื่อสร้างใหม่ mode = reset → แจ้งเตือนว่าโควตาสะสมจะหายไป
  - **ล็อคโหมด:** เมื่อแก้ไข rate เดิม → radio buttons โหมดจะถูก disabled
...
- **🆕 เลือกตาม rate (ไม่ใช่ product):**
  - Dropdown แสดงทุก rate: `[mode icon] ฿X/โควตา — [scope] — [period] [⏳ N รอยืนยัน]`
  - Option แรก: **"🔄 ทั้งหมด (รวมทุก rate)"** → aggregate ยอดจากทุก rate
  - Badge `⏳ N รอยืนยัน` แสดงจำนวน user ที่ยังไม่ confirm (confirm mode เท่านั้น)
- **🆕 Checkbox column (bulk confirm):**
  - แสดงเฉพาะเมื่อเลือก rate ที่เป็น confirm mode + `require_confirm=1`
  - **Header:** checkbox select-all (เลือก/ยกเลิก unconfirmed users ทั้งหมด)
  - **Body:** checkbox แต่ละแถว (เฉพาะ user ที่ยังไม่ confirm + มี pending quota > 0)
  - User ที่ confirm แล้ว แสดง ✅ icon แทน checkbox
- **🆕 Bulk confirm bar:**
  - แสดงเหนือตารางเมื่อเลือก user ≥ 1 คน
  - ปุ่ม "✅ ยืนยันโควตา N คน" → เรียก `bulkConfirmQuota` API
- **Period info:** แสดงเฉพาะเมื่อเลือก rate เดียว (ไม่แสดงใน mode "ทั้งหมด")
- ตาราง 9+ คอลัมน์: (checkbox), พนักงาน, ยอดขาย, โควตา(Auto), โควตา(Admin), รวม, ใช้แล้ว, คงเหลือ, การดำเนินการ
- **Auto column:** แสดง `(N รอ)` ใต้ตัวเลขสำหรับ unconfirmed users
- Modal: เพิ่มโควตาให้พนักงาน (จำนวน + หมายเหตุ)
- Modal: ดูประวัติ allocation ทั้งหมด
- **Confirm mode:** แสดงปุ่ม ✅ ยืนยัน (ทั้งรายบุคคลและ bulk) + ใช้ `bulkConfirmQuota` API
- **คงเหลือ:** แสดง `0 (หมดอายุ)` / `0 (ยังไม่เริ่มใช้)` ตามสถานะ
- **Data source:** ใช้ `getSummaryByRate(companyId, rateId | 'all')` แทน `getQuotaSummary`
...
### `SingleDatePicker.tsx` — เลือกวันเดียว

- **Props:** `value` (YYYY-MM-DD หรือ ''), `onChange(date)`, `placeholder?`, `disabled?`, `className?`
- **UI:** ปฏิทินเดือนเดียว + preset (วันนี้/พรุ่งนี้/ต้นเดือน/สิ้นเดือน/ต้นเดือนหน้า)
- **Click:** เลือกวันแล้วปิดทันที + ปุ่มล้าง
- **Popover:** `position: fixed` + `z-[9999]` — แสดงเหนือ modal ทุกชั้น

### `DateRangePicker.tsx` — เลือกช่วงวัน

- **Props:** `value: { start: string, end: string }` (ISO), `onApply(range)`
- **UI:** ปฏิทิน 2 เดือนเคียงกัน + preset (วันนี้/7วัน/30วัน/เดือนนี้/ฯลฯ)
- **Popover:** `position: fixed` + `z-[9999]` — แสดงเหนือ modal ทุกชั้น

> ⚠️ ทั้ง 2 components ใช้ `position: fixed` + คำนวณตำแหน่งจาก `getBoundingClientRect()` เพื่อให้ popover แสดงเหนือ modal overlay (`z-50`) ได้ — ไม่ถูก `overflow` ของ container บัง
...
| `getQuotaSummary(companyId, qpId)` | GET | `summary` | สรุปโควตาทุกคน (legacy, product-based) |
| `getSummaryByRate(companyId, rateId \| 'all')` | GET | `summary_by_rate` | **🆕 สรุปโควตาตาม rate** — รับ rate ID หรือ `'all'` (aggregate) |
| `bulkConfirmQuota(payload)` | POST | `bulk_confirm_quota` | **🆕 ยืนยันโควตาหลายคน** — `{ rateScheduleId, userIds[], confirmedBy, companyId }` |
