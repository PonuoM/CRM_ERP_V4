...
| สถานะ | เงื่อนไข SQL |
|-------|-------------|
| `incomplete` | `payment_status != 'Approved'` AND ยังไม่ stamp AND ไม่ใช่สถานะ (Returned, Cancelled, BadDebt) |
| `pending` | `payment_status = 'Approved'` AND ยังไม่ stamp AND ไม่ใช่สถานะ (Returned, Cancelled, BadDebt) |
| `calculated` | มี record ใน `commission_stamp_orders` (นับเสมอแม้จะมีการยกเลิกทีหลัง) |
