...
│                                                                  │
│  ┌─────────────────┐   ┌──────────────────┐   ┌──────────────┐  │
│  │ OrderTable      │   │ OrderManagement  │   │ Cancellation │  │
│  │ ปุ่มยกเลิก      │──▶│ Modal            │   │ Page          │  │
│  │ (CancelConfirm  │   │ (เปลี่ยนสถานะ +   │   │ Tab จัดประเภท │  │
│  │  Modal)          │   │  ระบุประเภท)      │   │ Tab ตั้งค่ายกเลิก│  │
│  └────────┬────────┘   └────────┬─────────┘   └──────┬───────┘  │
│           │                     │                     │          │
│           ▼                     ▼                     ▼          │
│  ┌─────────────────────────────────────────────────────────────┐ │
...
| `pages/CheckOrderPage.tsx` | หน้ารวม "ตรวจสอบคำสั่งซื้อ" — 5 tab: จัดประเภทยกเลิก + ตรวจสอบโปรโมชั่น + ตรวจสอบ Creator + ตั้งค่ายกเลิก + สั่งทำ |
...
- เมนู "ตรวจสอบคำสั่งซื้อ" → key: `nav.cancellation_management`
- อยู่ใต้กลุ่ม "คำสั่งซื้อและลูกค้า" ใน `components/Sidebar.tsx`
...
3. UI แสดง Card สีส้ม "ข้อมูลการยกเลิก" ด้านล่าง (full-width) ประกอบด้วย:
   - Grid 2 col: ประเภทการยกเลิก* | หมายเหตุ (ไม่บังคับ)
4. Validation: ต้องเลือกประเภท (ไม่ใช่ "ยังไม่ระบุ") ก่อนกดบันทึก
5. handleSave():
   a. ตรวจสอบ: ถ้า status=Cancelled แต่ไม่ได้เลือกประเภท
      → แสดง inline warning สีแดง + auto-scroll ไปที่ dropdown + return
   b. ถ้า status=Cancelled + มีประเภท → confirmCancellation()
   c. onSave(updatedOrder) → API patch
```

> **UI Details (OrderManagementModal):**
> - Section "การจัดส่ง" layout: สถานะออเดอร์ + เลข Tracking อยู่คู่กัน (grid 2 col)
> - เมื่อเลือกสถานะ "ยกเลิก" → Card สีส้ม (`bg-orange-50`, `border-orange-200`, `rounded-xl`) ปรากฏด้านล่าง
> - Header: icon ⚠ + "ข้อมูลการยกเลิก"
> - ประเภท dropdown มี `*` สีแดง (required), bg-white, border-orange-300
> - Error state: inline banner สีแดง (`bg-red-50`, `animate-pulse`) + dropdown ขอบแดง (`border-red-400`, `ring-red-200`)
> - Auto-scroll ด้วย `useRef` + `scrollIntoView({ behavior: 'smooth', block: 'center' })`
> - เมื่อเลือกประเภทแล้ว → error หายอัตโนมัติ

### Flow 3: จัดประเภท bulk (CheckOrderPage → Tab "จัดประเภทยกเลิก")
...
> **Backend Note (analyze_cancelled_orders.php):**
> - Suggested type ID ใช้ dynamic lookup จาก `cancellation_types` โดย label
> - Fallback ใช้ ID จริงจาก DB (`$types[0]`, `$types[1]`) ไม่ hardcode เป็น 1/2
> - ป้องกัน FK constraint violation เมื่อ ID ใน DB ไม่ตรงกับ seed data เดิม

### Flow 4: ตั้งค่าประเภทยกเลิก (CheckOrderPage → Tab "ตั้งค่ายกเลิก")
...
1. **เปลี่ยนสถานะเป็น Cancelled:** ต้องเลือกประเภทยกเลิก → inline warning + auto-scroll ถ้าไม่เลือก
2. **กดปุ่มยกเลิก:** ใช้ประเภท default อัตโนมัติ (lock, ไม่ให้เปลี่ยน)
3. **COD Validation:** ตรวจยอดเก็บเงินตรงกับยอดออเดอร์ก่อนบันทึก (billDiscount เป็น flat amount)
...
| ปัญหา | สาเหตุ | แก้ไข |
|---|---|---|
| FK constraint violation ตอนจัดประเภท bulk | `analyze_cancelled_orders.php` hardcode fallback ID=1,2 แต่ DB จริงมี ID อื่น | ใช้ dynamic fallback จาก `$types[0]`, `$types[1]` |
| ยอดรวมออเดอร์คำนวณผิด | `calculateOrderTotal` หาร `billDiscount` ด้วย 100 (คิดเป็น %) | แก้เป็น flat amount (ลบตรงๆ) |
| React key warning ใน OrderManagementModal | Items list ใช้ `key={item.id}` แต่ id อาจเป็น null | ใช้ `key={item.id ?? \`item-${index}\`}` |

## Deploy

1. รัน SQL migration: `api/Database/20260310_cancellation_types.sql`
2. เพิ่ม permission key `nav.cancellation_management` ใน role_permissions DB ให้ role ที่ต้องการเห็นเมนู
3. ตั้งค่าประเภทเริ่มต้นผ่านหน้า "ตรวจสอบคำสั่งซื้อ" → Tab "ตั้งค่ายกเลิก"
