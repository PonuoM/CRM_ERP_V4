...
2. **ระบบจับคู่รหัสสินค้าอัจฉริยะ (Smart SKU Mapping System)**
   ...
   - **ระบบเปิด/ปิดการแสดงผลอัจฉริยะ:** สามารถควบคุมการแสดงจำนวนสต็อก JST ในหน้าเลือกสินค้าได้อิสระรายบริษัท โดยการตั้งค่าคีย์ `SHOW_JST_INVENTORY_{company_id}` ในตาราง `env` เป็น `true`, `false`, `1`, `0` (ค่าเริ่มต้นคือเปิดแสดง)
...
5. **Frontend (`pages/jst/InventoryPage.tsx` & `pages/jst/CompanySettingsPage.tsx`)**
...
---
   ...
   - **ระบบแท็บ (Tab Navigation):** 
     - **แท็บรายสินค้า (Group by SKU)** และ **แท็บแยกตามคลัง:** ทุก Role ในระบบสามารถมองเห็นและใช้งานได้
     - **แท็บตั้งค่าบริษัท (Settings & Sync Health Dashboard):** จะมองเห็นเฉพาะผู้ที่มีสิทธิ์ระดับ System (`is_system = 1`) หรือ Admin เท่านั้น (ส่วน Dropdown การเลือกสลับบริษัทในแท็บตั้งค่า จะมองเห็นเฉพาะ Super Admin เท่านั้น)
