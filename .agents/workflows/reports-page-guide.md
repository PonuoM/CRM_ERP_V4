...
**ไฟล์หลัก:** `pages/ReportsPage.tsx`
**ฟังก์ชันส่งออก (Single Source of Truth):** `utils/exportOrdersFormat.ts` (ใช้งานร่วมกับ `pages/ManageOrdersPage.tsx`)
...
### 3.4 Order Boxes (สำหรับ Returned / Partial Return)
- หลังโหลด orders → batch-fetch `order_boxes` ของออเดอร์ทั้งหมด (chunks of 100) เพื่อจับเคสตีกลับบางกล่องที่สถานะบิลยังไม่ใช่ Returned
- API: `Orders/get_order_boxes.php?order_ids=ID1,ID2,...`
- เก็บ map `{orderId-boxNumber → return_status}` ใน state `orderBoxesMap`
- ใช้ใน `reportData` useMemo สำหรับคอลัมน์สถานะและยอดของ orders-raw export
...
**คอลัมน์ CSV:**
`วันที่สั่งซื้อ, เวลาสั่งซื้อ, เลขคำสั่งซื้อ, user_id, ผู้ขาย, แผนก, ชื่อลูกค้า, เบอร์โทรลูกค้า, ประเภทลูกค้า, วันที่จัดส่ง, ช่องทางสั่งซื้อ, เพจ, ช่องทางการชำระ, ที่อยู่, ตำบล, อำเภอ, จังหวัด, รหัสไปรษณีย์, ภาค, รหัสสินค้า/โปร, สินค้า, ประเภทสินค้า, ประเภทสินค้า (รีพอร์ต), ชื่อโปร, ของแถม, จำนวน (ชิ้น), ราคาต่อหน่วย, ส่วนลด, ส่วนลดประจำเดือน, ยอดรวมรายการ, ค่าจัดส่ง (ต่อบิล), ส่วนลดท้ายบิล, ยอดรวมทั้งบิล, ยอดรวมรายคน, หมายเลขกล่อง, หมายเลขติดตาม, วันที่จัดส่ง Airport, สถานะจาก Airport, สถานะออเดอร์, สถานะการชำระเงิน, สถานะสลิป, วันที่รับเงิน, ตะกร้าขาย, สาเหตุยอดไม่ตรง`

**Logic พิเศษ:**
- แต่ละ order แยก row ตาม item (1 row ต่อ 1 สินค้า) โดยจะเรียงลำดับสินค้าตามชื่อผู้ขาย (creator_id) เพื่อให้สินค้าที่ขายโดยคนเดียวกันอยู่ติดกัน
- **ส่วนลดประจำเดือน**: ค่าจาก `order_items.monthly_discount` ระดับรายการ (item-level)
- **ยอดรวมรายการ**: ค่าจาก `order_items.net_total` ตรงๆ ไม่คำนวณใหม่ — ถ้ากล่องของรายการนั้นมี `order_boxes.return_status` หรือออเดอร์ทั้งใบเป็น `Returned` ให้แสดง 0
...
- **ยอดรวมทั้งบิล (Total Bill)**: แสดงยอดรวมทั้งหมดของออเดอร์นั้นเฉพาะในบรรทัดแรกสุดของออเดอร์ ส่วนบรรทัดถัดไปจะแสดงเป็น `-` — ถ้ามีกล่องถูกตีกลับบางใบ (แม้สถานะออเดอร์ยังไม่ใช่ Returned) ใช้ผลรวม `net_total` ของกล่องที่เหลือ
- **ยอดรวมรายคน**: คำนวณยอดขายรวมของแต่ละคน (Sum by Creator) โดยจะแสดงยอดรวมเฉพาะในบรรทัดแรกของสินค้าที่ผู้ขายคนนั้นๆ ขายได้ในออเดอร์นั้น ส่วนบรรทัดถัดไปของผู้ขายคนเดิมจะแสดงเป็น `-` (หมายเหตุ: หากบิลนั้นมีค่าจัดส่งหรือส่วนลดท้ายบิล ระบบจะทำการหารเฉลี่ยและกระจายสัดส่วนตามมูลค่าสินค้าที่แต่ละคนขายได้ - Proportional Distribution) ไม่นับรายการในกล่องที่ถูกตีกลับ

**สถานะออเดอร์ (ตีกลับ):**
- ดึง `order_boxes` ของออเดอร์ทั้งหมด (ไม่จำกัดสถานะ Returned) แล้วแสดงสถานะ return ต่อกล่องเมื่อกล่องนั้นมี `return_status`
- กล่องที่มี `return_status` แสดง `ตีกลับ (กล่อง N : ...)` แม้สถานะออเดอร์ยังไม่ใช่ Returned (เช่น Delivered แต่ตีกลับบางกล่อง)
- กล่องที่ไม่มี `return_status` ยังตามสถานะออเดอร์ เช่น `จัดส่งสำเร็จ`
...
- **โครงสร้างคอลัมน์**: อ้างอิงตาม `orders-raw` ทั้งหมด (รวมถึง `ค่าจัดส่ง (ต่อบิล)`, `ส่วนลดท้ายบิล`, `ยอดรวมทั้งบิล`, `ยอดรวมรายคน`, และ `สาเหตุยอดไม่ตรง`) แต่จะต่อท้ายด้วยคอลัมน์สถานะค่าคอมและข้อมูลการ Stamp
