...
file_path = r"c:\laragon\www\CRM_ERP_V4\components\DistributionV2\PreviewModal.tsx"
...
pattern_input = r"(<input\s*type=\"text\"\s*id=\"sessionTag\"\s*className=\"[^\"]*\"\s*placeholder=\"[^\"]*\"\s*list=\"sessionTags\"\s*value=\{sessionTag\}\s*onChange=\{\(e\) => setSessionTag\(e\.target\.value\)\}\s*\/\>\s*<datalist id=\"sessionTags\">\s*\{sessionTagsList\.map\(tag => \(\s*<option key=\{tag\} value=\{tag\} \/>\s*\)\)\}\s*</datalist>)"
