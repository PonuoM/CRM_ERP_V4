...
import { DollarSign, ShoppingCart, TrendingUp, Users as UsersIcon, ChevronDown } from 'lucide-react';
...
const SalesDashboard: React.FC<SalesDashboardProps> = ({ orders = [], customers = [] }) => {
  const [month, setMonth] = useState<string>(() => String(new Date().getMonth() + 1).padStart(2, '0'));
  const [year, setYear] = useState<string>(() => String(new Date().getFullYear()));

  const { monthlySales, monthlyOrders, customersCount, performancePct } = useMemo(() => {
    ...
    const performancePct = performanceTarget > 0 ? (monthlySales / performanceTarget) * 100 : 0;

    return { monthlySales, monthlyOrders: monthlyOrders.length, customersCount, performancePct };
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <StatCard title="ยอดขายรายเดือน" value={`฿${monthlySales.toLocaleString()}`} subtext="อัปเดตล่าสุด" icon={DollarSign} />
        <StatCard title="จำนวนออเดอร์" value={monthlyOrders.toString()} subtext="ช่วงนี้" icon={ShoppingCart} />
        <StatCard title="ประสิทธิภาพ" value={`${performancePct.toFixed(2)}%`} subtext="เทียบเป้าหมาย" icon={TrendingUp} />
        <StatCard title="ลูกค้า" value={customersCount.toString()} subtext="ทั้งหมด" icon={UsersIcon} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          <h3 className="text-md font-semibold text-gray-700 mb-4">สถานะการชำระเงิน</h3>
