...
const CallDetailsPage: React.FC<CallDetailsPageProps> = ({ currentUser }) => {
  ...
  const fetchEmployeeCallData = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/index.php/call_overview?month=${selectedMonth}`, {
        method: 'GET',
        headers: {
  ...
  const filteredEmployees = employeeCallData.filter(employee =>
      ...
      <div className="w-full">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-gray-800">รายละเอียดการโทร</h1>
          </div>
          <p className="text-gray-600 ml-11">ข้อมูลการโทรของพนักงาน Telesale และ Super Telesale</p>
        </div>
                    ...
                    {filteredEmployees.length > 0 ? (
                      filteredEmployees.map((employee) => (
                        <tr key={employee.user_id} className="hover:bg-gray-50 transition-colors">
                          <td className="py-3 px-4 text-sm text-gray-800 font-medium">{employee.first_name}</td>
                        ...
                        <td colSpan={8} className="py-12 text-center">
                          <div className="flex flex-col items-center">
                            ...
                            <p className="text-gray-400 text-xs mt-1">
                              {searchTerm ? 'ลองปรับเปลี่ยนคำค้นหา' : 'ลองเลือกเดือนอื่นหรือกดปุ่ม "ดึงข้อมูล" เพื่อโหลดข้อมูลใหม่'}
                            </p>
                          </div>
                        </td>
                      </tr>
