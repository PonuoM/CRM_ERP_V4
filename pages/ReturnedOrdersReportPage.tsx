...
const ReturnedOrdersReportPage: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!start || !end) {
        toast.error('แจ้งเตือน', 'กรุณาระบุช่วงวันที่เพื่อดูรายงาน (ไม่อนุญาตให้ค้นหาทั้งหมด)');
        setLoading(false);
        return;
      }
      ...
      if (diffDays > 186) { // ~6 months
        toast.error('แจ้งเตือน', 'กรุณาเลือกช่วงเวลาไม่เกิน 6 เดือน เพื่อป้องกันโหลดข้อมูลหนักเกินไป');
        setLoading(false);
        return;
      }
      
      let query = `returned_orders_report?status_type=${activeTab}&resolution_status=${resolutionFilter}&start_date=${start}&end_date=${end}`;
      if (userId) {
