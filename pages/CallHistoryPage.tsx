...
const CallHistoryPage: React.FC<CallHistoryPageProps> = ({
  ...
  const [pausedAudios, setPausedAudios] = useState<Map<number, number>>(
    new Map(),
  );
  ...
  const playRecording = async (recordingURL: string, id: number) => {
    ...
    if (audioRef.current && currentPlayingId !== null) {
      ...
      setPausedAudios((prev) =>
        new Map(prev).set(currentPlayingId, pausedPosition),
      );
    ...
    try {
      ...
      if (
        ...
        if (isDualRequest) {
          ...
          setFilteredRecordings(pageObjects);
          ...
          setNextPageUri("");
          setPrevPageUri("");
          setLimitReached(endIndex >= uniqueObjects.length);
        } else {
          // Single request
        ...
        if (
          ...
          const agentPhone =
            currentUser.role === UserRole.Supervisor && selectedAgent
              ? selectedAgent
              : currentUser.phone;
          ...
          if (agentPhone) {
            ...
            // Remove duplicates based on recording ID
            const uniqueObjects = mergedData.objects.filter(
              (obj: any, index: number, self: any[]) =>
                index === self.findIndex((t: any) => t.id === obj.id),
            );
    ...
    try {
      ...
      if (authResult.success && authResult.token) {
        ...
        if (
          ...
          // Update page in all parameter sets
          paramsList.forEach((paramObj) => {
            paramObj.page = page;
            paramObj.pagesize = pageSize; // Keep the current pageSize
          });
          ...
          if (isDualRequest) {
            ...
            // Calculate pagination for this page
          ...
          const apiConfig: any = {
            ...
            sort: "",
            page: page,
            pagesize: pageSize, // Keep the current pageSize
            maxresults: -1,
            includetags: true,
            includemetadata: true,
            includeprograms: true,
          ...
          params.append(
            "includemetadata",
            apiConfig.includemetadata.toString(),
          );
          params.append(
            "includeprograms",
            apiConfig.includeprograms.toString(),
          );
          ...
          if (
            ...
            if (agentPhone) {
              ...
              const startIndex = (page - 1) * pageSize;
              const endIndex = startIndex + pageSize;
              const pageObjects = mergedData.objects.slice(
                startIndex,
                endIndex,
              );
              ...
              const paginatedData = {
                ...
                page: page,
                pageSize: pageSize,
                resultCount: mergedData.objects.length,
              };

              setRecordingsData(paginatedData);
              setFilteredRecordings(pageObjects);
              setSearchedAgent(selectedAgent);
              setCurrentPage(page);
              setPageSize(pageSize);
              ...
              setNextPageUri("");
              setPrevPageUri("");
              setLimitReached(endIndex >= mergedData.objects.length);
            }
          } else {
            // Normal request
            const response = await fetch(searchUrl, {
  ...
  const fetchNextPage = async () => {
    if (nextPageUri && !limitReached && !isSearchLoading) {
      ...
      try {
        // Use proxy URL
        const proxyUrl = nextPageUri.replace(
          "https://onecallvoicerecord.dtac.co.th",
          "/onecall",
        );
  ...
  const fetchPrevPage = async () => {
    if (prevPageUri && !isSearchLoading) {
      ...
      try {
        // Use proxy URL
        const proxyUrl = prevPageUri.replace(
          "https://onecallvoicerecord.dtac.co.th",
          "/onecall",
        );
        ...
        <div className="w-full">
          ...
          <div className="mb-6">
            ...
            <p className="text-gray-600 ml-11">
              ค้นหาและดูประวัติการโทรทั้งหมดในระบบ
            </p>
          </div>
              ...
              {currentUser.role === UserRole.Supervisor ? (
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                  >
                    <option value={currentUser.phone || ""}>
                      {currentUser.firstName} (ตัวคุณ)
                    </option>
