-- ============================================
-- SQL Script: Update Customer Statistics (Batch Mode)
-- Description: อัปเดตทีละส่วนเพื่อป้องกัน Lock Timeout
-- ============================================

DELIMITER $$

DROP PROCEDURE IF EXISTS UpdateCustomerStatsBatch$$

CREATE PROCEDURE UpdateCustomerStatsBatch()
BEGIN
    ...
    -- คำนวณสถิติลงตารางชั่วคราวทีเดียว (ไม่มี lock ตารางจริง)
    -- Calls
    ...
    WHILE offset_val < total_rows DO
        ...
        -- DO SLEEP(0.1); -- พักนิดนึงให้ระบบหายใจ (ถ้าจำเป็น)
    END WHILE;
...
-- เรียกใช้งาน Procedure
CALL UpdateCustomerStatsBatch();

-- ลบ Procedure เมื่อเสร็จ
DROP PROCEDURE IF EXISTS UpdateCustomerStatsBatch;
