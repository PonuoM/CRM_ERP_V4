...
- **Recipient Phone Sync (Fallback Safety)**: To remain fully backwards compatible with diverse `api/index.php` validations, any user edits inside the `OrderManagementModal` for `recipientPhone` are mirrored simultaneously to three paths: `shippingAddress.recipientPhone` (native), `shippingAddress.phone` (backend strictly checks this), and the root payload layer `recipient_phone`. This guarantees no data loss when pushing `PUT` payloads to the endpoints.
...
| `calculateOrderTotal(items, shipping, discount, boxes)` | `utils/orderCalculations.ts` | COD validation in `handleSave` + auto-updating `totalAmount` on item changes |
...
3. **Exclude items in returned boxes** (`calculateOrderTotal` only): If an item belongs to a box where `returnStatus === 'returned'` or `status === 'RETURNED'`, its value is calculated as 0 to ensure the order total reflects only non-returned goods.
...
### Save Button UI
- **Visibility**: The "บันทึกการเปลี่ยนแปลง" (Save) button is strictly tied to the `isEditing` state. It is only visible when the user is actively in editing mode.
- **Loading State (`isSaving`)**: Clicking Save triggers a loading spinner and disables the button to prevent double submissions. The spinner persists until all API calls and validations complete.
