...
- **ช่องค้นหารองรับการค้นหาด้วย**: ชื่อ, นามสกุล, รหัสลูกค้า, เบอร์โทรหลัก, และ **เบอร์สำรอง** (ตัวระบบค้นหาจะตัดเลข 0 นำหน้าให้ ทำให้ค้นหาเจอแมัพิมพ์ไม่ครบ)
- แสดงผลลัพธ์ dropdown, เมื่อเลือก → fetch ข้อมูลสดจาก `GET /customers/{id}`

### 2.2 สร้างลูกค้าใหม่
- **เงื่อนไขสิทธิ์**: ปุ่ม "ไม่พบลูกค้านี้ในระบบ? สร้างรายชื่อใหม่" จะถูกซ่อนสำหรับผู้ใช้ที่มี Role เป็น `Telesale` หรือ `Supervisor Telesale`
- ถ้าพิมพ์เบอร์โทร (0xxxxxxxxx) → ใส่ในช่อง phone
- ถ้าพิมพ์ชื่อ → แยกเป็น firstName / lastName
- การกรอกเบอร์โทรหลัก (`newCustomerPhone`): ต้องขึ้นต้นด้วย `0` เสมอ และมีความยาว 9-10 หลัก (เพื่อรองรับเบอร์บ้านและเบอร์มือถือ)
- กรอก: ชื่อ, นามสกุล, เบอร์โทร, เบอร์สำรอง (เพิ่มได้สูงสุด 3 เบอร์), ประเภทลูกค้า

### 2.3 แก้ไขลูกค้า (เมื่อเลือกลูกค้าเดิม)
- แก้ไข: ชื่อ, นามสกุล, เบอร์โทร, เบอร์สำรอง (เพิ่มได้สูงสุด 3 เบอร์), ประเภทลูกค้า, Facebook, Line ID
...
- **ที่อยู่ profile** — ดึงจาก `customer.address` (default)
  - เมื่อเลือกลูกค้า (`handleSelectCustomer`) → `normalizeAddress(customer.address)` จะดึง `recipient_phone` จาก address object
  - ถ้า address ไม่มี `recipient_phone` → fallback เป็น `customer.recipient_phone` → `customer.phone`
  - phone ถูก set **ครั้งเดียว** ตอนเลือกลูกค้า/เปลี่ยนที่อยู่ → user แก้ไข/ลบได้อิสระ (ไม่มี runtime fallback ใน value prop)
- **ที่อยู่ที่บันทึกไว้** — ดึงจาก `Address_DB/get_address_data.php?endpoint=customer_addresses` ดึงทั้งชื่อผู้รับ, เบอร์ผู้รับ (`recipient_phone`) มาร่วมแสดงผล
- **ที่อยู่ใหม่** — กรอกใหม่ทั้งหมด pre-populate เบอร์โทรจาก `customer.phone` (เบอร์หลัก)
...
- **ตั้งเป็นที่อยู่หลัก** → `POST endpoint=set_primary_address`
  - **Backend**: สลับข้อมูลระหว่าง `customers` ↔ `customer_address` ครบทุก field:
    - `street`, `province`, `district`, `subdistrict`, `postal_code`
    - `recipient_first_name`, `recipient_last_name`, `recipient_phone`
  - **Frontend** (`handleSetPrimaryAddress`):
    1. `setSelectedCustomer` — อัปเดต `address` + `recipient_phone` ด้วยข้อมูลจากที่อยู่สำรองที่ถูกเลื่อนขึ้น
    2. `setShippingAddress` — ส่ง `phone` ผ่าน `normalizeAddress` ให้เบอร์ผู้รับอัปเดตทันที
    3. `setAddressOptions` — ย้ายที่อยู่หลักเดิมเข้า list พร้อม `recipient_phone`
    4. ⚠️ **ห้ามเรียก `handleAddressOptionChange("profile")`** หลัง setState เพราะ React setState เป็น async → `selectedCustomer` ยังเป็นค่าเก่า จะ overwrite `shippingAddress` กลับเป็นข้อมูลเดิม
...
| `newCustomerPhone` | ต้องกรอกเบอร์โทร (ลูกค้าใหม่) โดยต้องขึ้นต้นด้วย `0` และมีความยาว 9-10 หลัก |
| `backupPhone` | เบอร์สำรองรองรับได้ 3 เบอร์ หากกรอก จะตรวจสอบ format เบอร์แยกแต่ละอัน (ต้องขึ้นต้นด้วย `0` และมีความยาว 9-10 หลัก) |
| `shippingAddress` | ต้องมีที่อยู่จัดส่ง + ตรวจความสัมพันธ์ผ่าน `check_exist.php` |
| `customerStatus` | ต้องเลือกสถานะลูกค้า |
| `deliveryDate` | ต้องระบุวันจัดส่ง (ไม่เกินวันที่ 5 ของเดือนถัดไป) ⚠️ คำนวณ maxDate ด้วย local date format — **ห้ามใช้ `toISOString()`** เพราะ UTC จะเลื่อนวันถอยหลัง 1 วันใน GMT+7 |
| `items` | ต้องมีสินค้าอย่างน้อย 1 รายการ |
| `quota` | ตรวจสอบโควตาคงเหลือก่อนบันทึก |
| `paymentMethod` | ต้องเลือกวิธีชำระ |
| `transferSlips` | ถ้า Transfer → ต้องมีสลิป + ธนาคาร + วันที่โอน + จำนวนเงิน (ถ้ายอดโอนรวมไม่ตรงกับ `totalAmount` ต้องระบุสาเหตุ หากไม่ระบุจะถูกบล็อคการบันทึกและปลดล็อคปุ่ม `isSaving = false` ทันที) |
...
```ts
{
  ...
  updateCustomerInfo?: {...},    // อัปเดต firstName, lastName, phone, backupPhone (array ทั้ง 3 รายการถูก join เป็น comma-separated string)
  ...
  slipUploads?: [{               // Array of Objects สำหรับการอัปโหลดสลิป
    dataUrl, 
    bankAccountId, 
    transferDate, 
    amount, 
    mismatch_reason              // สาเหตุที่ยอดสลิปขาด/เกิน (ถ้ามี)
  }],
