...
## 31. อัปเดตล่าสุด: การทำงานนำเข้า COD ควบคู่กับระบบตีกลับ (31/03/2026 - Part 2)

### การชุบชีวิตออเดอร์ตีกลับผ่านหน้า COD Management
เมื่อบัญชีทำรายการนำเข้ายอดเก็บเงิน COD (COD Management) หากพบ Tracking Number ที่มีสถานะเป็น "ตีกลับแล้ว" (`status = 'RETURNED'`) เกิดขึ้นในระบบ:

- ระบบในหน้า COD จะไม่บล็อกทิ้งทันที แต่จะขึ้นสถานะเป็น 🔴 **ตีกลับแล้ว (สามารถติ๊ก "ข้าม" เพื่อยกเลิกตีกลับและนำเข้าได้)**
- หากแอดมิน **ติ๊กกล่อง "ข้าม" (Force Import)** และกดตกลงนำเข้า:
  1. ก่อนบันทึกไฟล์ COD ระบบเบื้องหลังจะเรียก API `revert_returned_order.php` เพื่อ **ล้างข้อมูลการตีกลับของกล่องนั้นทิ้งทั้งหมด** (ล้าง `return_status`, `return_note`, `return_created_at`)
  2. ดึงสถานะของกล่อง (`order_boxes.status`) และออเดอร์แม่ (`orders.order_status`) ย้อนกลับมาเป็น **`Pending`** อัตโนมัติ (แทนที่จะเป็น `Shipping` แบบเมื่อก่อน)
  3. ดำเนินการจับยอดเงิน COD ใส่เข้าไปใน `collected_amount` ได้อย่างสมบูรณ์โดยที่ Trigger DB ไม่ฟ้อง Error

**ผลกระทบต่อหน้าระบบจัดการตีกลับ (Return Management):**
- ออเดอร์ที่ถูกกู้คืนผ่านหน้า COD ในกรณีนี้ จะ **หายไปจากแท็บรายการตีกลับ** โดยอัตโนมัติ (เสมือนเพิ่งส่งของสำเร็จ) ช่วยลดข้อผิดพลาดการทำงานซ้ำซ้อนระหว่างแผนกโกดังและบัญชีครับ


## 31. ???????????? (Change Log - 06/05/2026)

### Refactoring: Best Practice for save_return_orders.php
- **Pessimistic Locking (Row Locking):** ????? FOR UPDATE ?? SQL Query ??????????????????????????????????? ????????????????? Race Condition ????????????????????????????????/???????????????????????
- **N+1 Query Resolution:** ????? $orderCache ?????????????????????????????????? (total boxes, returned boxes) ???????? Query ???????????????????????????????????????????????????????????
- **Pre-compiled Prepared Statements:** ???? \->prepare() ??????????????????? ??????????????????????????????????????? Batch

### UI Changes: ???????????
- **?????????:** ??? pending (???????????) ??? delivered (????????????) ???????????????????? ????????? / ???????????? (??? 
ull ??????) ????????????? 2 ??????? Action ??????????????????????? (Undo) ????????????????????????????? Main Order ???

- **Dropdown Options (Undo Return Modal):** ?????????????????? Dropdown ????????????????????????????????????????????????? ??????????????????????????????????? (???????????, ??????????????, ???????????, ?????????, ??????, ???????) ????????????????????????? OrderManagementModal

- **Wording Change:** ???????????? '????????? / ????????????' ???? '???????????? / ????????????' ?????? ReturnManagementPage.tsx ??? OrderManagementModal.tsx ??????????????????????????
