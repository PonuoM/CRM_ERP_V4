...
- **ยังไม่ตรวจสอบ (Pending)**: แสดงรายการคำสั่งซื้อที่มีสถานะ `Returned` แต่ยังไม่มีปรากฏในตาราง `order_returns`
  - ใช้สำหรับตรวจสอบรายการที่ขนส่งตีกลับมาใหม่
- **ตรวจสอบแล้ว (Verified)**: แสดงรายการที่ถูกบันทึกลงในตาราง `order_returns` แล้ว
  - ใช้สำหรับดูประวัติการตรวจสอบย้อนหลัง

### 2.3 ข้อกำหนดของไฟล์นำเข้า (Import & Paste Requirements)
...
### 2.4 ตรรกะการจับคู่ (Matching Logic)
...
### 2.5 สถานะการตรวจสอบ (Verification States)
...
- ระบบจะบันทึกข้อมูลรายการที่ตรวจสอบแล้ว (Order ID, Returns Amount, Note) ลงในตาราง `order_returns`
...
CREATE TABLE `order_returns` (
  ...
  `order_id` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'รหัสคำสั่งซื้อ',
